...
export function Clock({ time, city, timezone, date, flag, isLocal = false }: ClockProps) {
  // Parse time string to get hours, minutes, seconds
  const { hours, minutes, seconds, hourDegrees, minuteDegrees, secondDegrees } = useMemo(() => {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="text-lg font-semibold text-white tracking-wide">{city}</h3>
        {isLocal && (
      ...
      <div className="relative w-36 h-36 mb-4">
        ...
        <div className={`absolute inset-0 rounded-full border-2 ${
          ...
          {[...Array(12)].map((_, i) => (
            ...
            >
              <div className={`absolute top-2 left-1/2 -translate-x-1/2 w-0.5 h-2 rounded-full ${
          ...
          {/* Center dot */}
          <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3 h-3 rounded-full z-20 ${
        ...
        {/* Outer glow */}
        <div className={`absolute -inset-1 rounded-full blur-md opacity-20 ${
      ...
      <div className="font-mono text-3xl font-bold tracking-wider">
        ...
        <span className={isLocal ? 'text-emerald-300' : 'text-cyan-300'}>{String(seconds).padStart(2, '0')}</span>
      </div>
