...
export function useVideoSync() {
  ...
  const [syncStatus, setSyncStatus] = useState<
    "synced" | "ahead" | "behind" | "seeking" | "buffering"
  >("synced");
    ...
    const handleVideoState = (state: VideoState) => {
      ...
      if (rtt > 0 && rtt < 5000) {
        ...
        latencySamples.current = [
          ...latencySamples.current.slice(-(SYNC_CONFIG.LATENCY_SAMPLES - 1)),
          rtt / 2,
        ];
        const avgLatency =
  ...
  const syncVideoPlayback = useCallback(() => {
    ...
    smoothedDrift.current =
      smoothedDrift.current * (1 - SYNC_CONFIG.DRIFT_SMOOTHING) +
      rawDrift * SYNC_CONFIG.DRIFT_SMOOTHING;
    const currentDrift = smoothedDrift.current;

    let newSyncStatus: "synced" | "ahead" | "behind" | "seeking" | "buffering" =
      "synced";
    ...
    // Progressive rubber-banding for smaller drift
      ...
      if (currentDrift > 0) {
        ...
        newPlaybackRate = Math.min(
          1 + correctionFactor,
          SYNC_CONFIG.MAX_SPEEDUP,
        );
      } else {
  ...
  return {
    ...
    currentUser: currentUser
      ? { id: currentUser.id, city: currentUser.city, flag: currentUser.flag }
      : null,
    users,
    syncStatus,
    drift,
